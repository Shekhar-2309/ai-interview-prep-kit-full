...
- **Scraping:** `cheerio` for parsing + a custom link-ranking crawler (no headless browser —
...
- **Frontend and backend are separate deployables** (`web/` is its own Next.js app; the
  ...
  cleanly onto free-tier hosting: Vercel for Next.js, Render for the API — each optimized
...
credit card and no per-token billing — it's rate-limited only (30 RPM, ~12k TPM for this
...
LLM call in this process through **one shared, process-wide queue** — not per-request, not
...
- **Proactive throttling** — every response's `x-ratelimit-remaining-*` headers are read
  and used to self-pace *before* hitting a wall, not just react after a 429.
- **Reactive backoff** — an actual 429 triggers exponential backoff + jitter, respecting
...
- **Backend → Render.** `render.yaml` at the repo root defines the service (free plan,
  ...
  Vercel URL) as secrets in the Render dashboard — they're deliberately not synced from
  `render.yaml` since they're credentials.
- **Frontend → Vercel.** Standard zero-config Next.js deploy from the `web/` directory. Set
...
production (`server/middleware/auth.ts`) — `"lax"` works perfectly in local dev and
...
re-implements pipeline logic — the web route just adds persistence and SSE progress on top,
...
No hardcoded path list for the hiring page — the brief is explicit that this is
...
2. **Stage 1 scoring** (cheap, pre-fetch): every discovered link is scored by URL path and
   ...
   page-count budget — promising links get fetched first.
...
search provider in this pass — see Known Limitations.
...
1. **Extract requirements** from the pasted JD (no retrieval needed — it's already text)
2. **Crawl the company site** (a homepage needs crawling before it's useful)
3. **Search for public interview discussion**
4. **Generate the company brief** from whatever was actually found — skips the LLM call
...
5. **Generate questions, per category, per requirement subset** — technical and
...
6. **Coverage-check loop** — deterministic gap-finding (`coverageCheck.ts`), then a
...
8. **Build the schedule** — deterministic, no LLM
...
already covered by locked items — avoiding duplicate coverage and never silently
...
Reordering is plain array order (the index *is* the order — no separate `order` field to
...
helps"), and stripped via `kitSchema.ts`'s `toAppendixA()` before batch output — so the
...
contiguous chunks — chunk 1 (highest weight) lands on day 1. This is deliberately simpler
than a per-day time-budget optimizer: the tool is a triage aid, not a scheduling solver, and
contiguous-chunk allocation is easy to explain and verify (and to unit test — see
...
none of those answer the question a candidate actually has the night before — "if I only
have twenty minutes left, what should I look at?" Nothing else in the app answers that
directly.

**How it works:** `core/generation/weakSpots.ts` is a pure function (no LLM call — same
...
- **Coverage** — is this requirement backed by a question at all?
- **Practice confidence** — did the candidate mark low confidence (or never practice) the
...
free to recompute every time — no LLM budget spent on something that's fundamentally a
sort.
...
- **Flashcards are derived deterministically from questions**, not separately generated by
  ...
  calls against a tight TPM budget — but it's a purist-vs-pragmatist trade-off. A stricter
...
- **The coverage loop only forces retries on must-have gaps**, not nice-to-have gaps —
...
- **A kit owned by another user 404s, not 403s** — an unauthorized caller can't even
...
- **Duplicate-submission handling** (Section 10): identical `(jd, companyUrl, days)` input
  ...
  reuses the existing `GenerationRun` instead of racing it — the second request joins the
...
- **SSE progress is single-instance.** The in-memory pub/sub hub (`sseHub.ts`) only works
  within one server process — fine for a single free-tier Render instance, but wouldn't
...
- **Ownership, duplicate-run-reuse, and version-conflict logic in `kitService.ts` /
  `kitBuilderService.ts` are only exercised by inspection, not by integration tests** — they
